...
import (
	...
	"github.com/old-juniors/qltrq/server/config"
	"github.com/old-juniors/qltrq/server/events"
	"github.com/old-juniors/qltrq/server/github"
	"github.com/old-juniors/qltrq/server/server"
	"github.com/old-juniors/qltrq/server/tunnel"
)
...
func (j *Jprq) servePublicConn(conn net.Conn) error {
	...
	if !found {
		writeResponse(conn, 404, "Not Found", "tunnel not found. create one at qltrq.old-juniors.uz")
...
func (j *Jprq) serveEventConn(conn net.Conn) error {
	...
	if _, found := j.allowedUsers[user.Login]; !found {
		return events.WriteError(conn, "'qltrq' is invite-only service %s\n", "\n\tMembership - https://t.me/+rgoKcEj4FhplZmY6 \n")
	}
